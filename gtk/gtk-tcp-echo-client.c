...
void socket_recv_msg(GtkWidget *widget, gpointer *entry)
{
	int n;
...
int main(int argc, char *argv[])
{
	...
	GtkWidget *button;
	GtkWidget *entry_msg;
	GtkWidget *entry_addr, *entry_port;
	...
	gtk_box_pack_start(GTK_BOX(vbox), entry_msg, TRUE, TRUE, 0);
	
	button = gtk_button_new_with_label("Send");
	gtk_box_pack_start(GTK_BOX(vbox), button, TRUE, TRUE, 0);
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(socket_send_msg),
			 entry_msg);
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(socket_recv_msg),
			 entry_msg);
